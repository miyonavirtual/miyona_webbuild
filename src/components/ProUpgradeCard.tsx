...
import { Sparkles, CreditCard, CheckCircle2, ChevronRight } from "lucide-react";
...
export function ProUpgradeCard({ className = "" }: { className?: string }) {
    ...
    if (showPayment) {
        return (
            <div className={`p-5 rounded-3xl bg-purple-950/20 backdrop-blur-xl border border-white/10 shadow-[0_0_40px_rgba(100,20,180,0.15)] flex flex-col gap-4 w-[280px] text-white overflow-hidden transition-all duration-500 ${className}`}>
                <div className="flex items-center justify-between">
                    <h3 className="font-bold text-[15px] flex items-center gap-2">
                        <CreditCard className="w-4 h-4 text-purple-400" /> Payment
                    </h3>
                    <button onClick={() => setShowPayment(false)} className="text-[11px] text-purple-400 hover:text-white px-2 py-1 rounded bg-white/5 hover:bg-white/10 transition-colors">Back</button>
                </div>
    ...
    return (
        <div className={`p-5 rounded-3xl bg-purple-950/20 backdrop-blur-xl border border-white/10 shadow-[0_0_40px_rgba(100,20,180,0.15)] flex flex-col gap-4 w-[280px] text-white overflow-hidden transition-all duration-500 ${className}`}>
            ...
            <Button 
                ...
                className="w-full bg-purple-600 hover:bg-purple-500 text-white shadow-[0_0_15px_rgba(140,50,250,0.3)] rounded-xl h-10 mt-1 transition-all disabled:opacity-50 disabled:hover:bg-purple-600"
            >
                Upgrade to Pro
            </Button>
        </div>
